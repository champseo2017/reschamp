...
class Resume extends Component {
  ...
  render() {
    var {Educationth,schoolth,degreeth,graduatedth} = this.state
    if(this.props.data){
      var skillmessage = this.props.data.skillmessage;
      var education = this.props.data.education.map(function(education){
        return <div key={education.school}><h3>{schoolth}</h3>
        <p className="info">{degreeth} <span>&bull;</span><em className="date">{graduatedth}</em></p>
